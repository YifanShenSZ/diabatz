...
argparse::ArgumentParser parse_args(const size_t & argc, const char ** & argv) {
    ...
    parser.add_argument("--memory",             1, true, "memory of LBFGS, default = 10");
...
int main(size_t argc, const char ** argv) {
    ...
    size_t memory = 10;
    if (args.gotArgument("memory")) memory = args.retrieve<size_t>("memory");
    if (optimizer == "LBFGS") std::cout << "Set memory to " << memory << '\n';
    size_t max_iteration = 100;
    if (args.gotArgument("max_iteration")) max_iteration = args.retrieve<size_t>("max_iteration");
    std::cout << '\n';
    train::line_search::initialize(regset, degset, energy_set);
    train::line_search::optimize(regularized, optimizer, learning_rate, memory, max_iteration);
