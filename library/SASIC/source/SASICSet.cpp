...
: tchem::IC::IntCoordSet(format, IC_file) {
    ...
    if (! ifs.good()) throw CL::utility::file_error(SAS_file);
        ...
        while (true) {
            ...
            "SASIC::SASICSet::SASICSet: wrong input format in " + SAS_file);
...
std::vector<at::Tensor> SASICSet::operator()(const at::Tensor & q) {
    ...
    "SASIC::SASICSet::operator(): q must be a vector");
    if (q.size(0) != this->size()) throw std::invalid_argument(
    "SASIC::SASICSet::operator(): inconsisten dimension between q and this internal coordinate system");
    // Nondimensionalize
